import type { GithubIconOptions, GithubLinkOptions, ResolvedExtraLink } from '../types'
...
function buildGithubPrefixHtml(options: GithubLinkOptions): string {
  ...
  const commonStyle = `display:inline-block;vertical-align:text-bottom;width:${size};height:${size};margin-right:0.3em;`

  if (icon.asClass) {
    const className = icon.value || 'i-mdi-github'
    return `<span class="${className}" style="${commonStyle}"></span>`
  }

  const iconValue = icon.value || GITHUB_ICON_DATA_URL
  const safeIconValue = iconValue
    .replaceAll('"', '%22')
    .replaceAll('\'', '%27')
  const style = `${commonStyle}background-image:url("${safeIconValue}");background-size:contain;background-repeat:no-repeat;background-position:center;`
...
export class GithubLink extends AbstractLink {
  ...
  protected handle(raw: string, repoRaw?: string, aliasRaw?: string): ResolvedExtraLink | null {
    ...
    return {
      ...
      prefixHtml: buildGithubPrefixHtml(options)
    }
  }
}
